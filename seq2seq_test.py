...
embedding_path = dir_path + 'embeddings.pkl'
train_source_file = dir_path + 'text_files/source_train.txt'
train_target_file = dir_path + 'text_files/target_train.txt'
test_source_file = 'text_files/source_test.txt'
test_target_file = 'text_files/target_test.txt'
...
#dec_outputs, dec_memory = tf.nn.seq2seq.embedding_attention_seq2seq(enc_inp, dec_inp, cell, vocab_size, vocab_size, embedding_dim, feed_previous=pred)
...
X_test, Y_test = prep_data.prep_train(vocab, max_seq_length, test_source_file, test_target_file)
X_test = X_test[:64]
Y_test = Y_test[:64]
print "Loaded"
load_model_path = '/scratch/chemical/dual/ch7130186/Assignment_2/models/mem_300_GRU_emb_autoenc_86_0.019634919.ckpt'
...
out = [logits_t.argmax(axis=1) for logits_t in dec_outputs_batch]
invv = {vocab[v]:v for v in vocab.keys()}
print "Done"

X = X.T
out = np.array(out).T
def orig_pred(i):
	print('original : ' + ' '.join([invv[v] for v in X[i] if invv[v]!='</S>']))
	print('output   : ' + ' '.join([invv[v] for v in out[i] if invv[v]!='</S>']))
